...
st.caption("Mobielvriendelijke woning- en beleggingsplanner met scenario's (5% & 7%).")
...
with col1:
    st.metric("Woningfonds maand 24 (gem.)", f"{df_avg.loc[df_avg['Maand']==months_phase1, 'Woningfonds'].values[0]:,.0f} EUR")
with col2:
    st.metric("Belegging maand 24 (gem.)", f"{df_avg.loc[df_avg['Maand']==months_phase1, 'Belegging'].values[0]:,.0f} EUR")
with col3:
    st.metric("Totaal nominaal maand 24 (gem.)", f"{df_avg.loc[df_avg['Maand']==months_phase1, 'Totaal (nominaal)'].values[0]:,.0f} EUR")
with col4:
...
chart_df = pd.DataFrame({
    "Maand": df_avg["Maand"],
    "Gemiddeld (5%)": df_avg["Woningfonds"],
    "Goed (7%)": df_good["Woningfonds"],
}).set_index("Maand")
...
chart_df2 = pd.DataFrame({
    "Maand": df_avg["Maand"],
    "Gemiddeld (5%)": df_avg["Belegging"],
    "Goed (7%)": df_good["Belegging"],
}).set_index("Maand")
...
chart_df3 = pd.DataFrame({
    "Maand": df_avg["Maand"],
    "Gemiddeld (5%)": df_avg["Totaal (nominaal)"],
    "Goed (7%)": df_good["Totaal (nominaal)"],
}).set_index("Maand")
st.line_chart(chart_df3)

st.subheader("🛡️ Koopkracht (reëel, gecorrigeerd voor inflatie)")
chart_df4 = pd.DataFrame({
    "Maand": df_avg["Maand"],
    "Gemiddeld (5%)": df_avg["Totaal (reëel)"],
    "Goed (7%)": df_good["Totaal (reëel)"],
}).set_index("Maand")
...
st.info("Tip: open deze link op je telefoon en kies 'Toevoegen aan startscherm' voor een app-ervaring.")
